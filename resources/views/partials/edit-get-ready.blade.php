<div class="get-ready">
    ...
    <div id="get-ready" class="collapse">
          ...
          <div class="main-group">
              @if($get_task->video)
              <video width="100%" controls style='border: 1px solid #e8eaec;' poster="/thumbnails/{{$get_task->thumbnail}}">
                  <source src="/videos/{{$get_task->video}}" type="video/mp4">
                  <source src="/videos/{{$get_task->video}}" type="video/avi">
                  <source src="/videos/{{$get_task->video}}" type="video/ogg">
              </video>
              @elseif($get_task->thumbnail)
                  <img src="/thumbnails/{{$get_task->thumbnail}}" class="" style="max-width:100%;margin:auto;display:block">
              @endif
          </div>
          <div class='main-group form-group center'>
            <div class="dropzone" id="myAwesomeDropzone_ready"></div>
            <input class="ready_video_upload" type="hidden" name="ready_video_upload" value="{{$get_task->video}}">
            <input class="ready_thumb_upload" type="hidden" name="ready_thumb_upload" value="{{$get_task->thumbnail}}">
          </div>   
    </div>
</div>
